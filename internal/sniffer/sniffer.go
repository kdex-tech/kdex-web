...
func (s *RequestSniffer) parseRequestIntoAPI(
	...
	path string,
	operationId string,
) (*kdexv1alpha1.KDexOpenAPIInternal, error) {
	api := &kdexv1alpha1.KDexOpenAPIInternal{}
	...
	if r.ContentLength > 0 || r.Header.Get("Content-Type") != "" {
		...
		if requestSchemaRef != "" {
			...
			if api.Schemas == nil {
				api.Schemas = map[string]openapi.Schema{}
			}
			api.Schemas[requestSchemaRef] = *schema
		} else {
			schemaRef = &openapi.SchemaRef{
				Value: schema,
			}
...
func (s *RequestSniffer) sniff(r *http.Request) (*kdexv1alpha1.KDexFunction, error) {
	...
	api, err := s.parseRequestIntoAPI(r, name, method, path, operationId)
	...
	s.mergeAPIIntoFunction(fn, api, path, r.Header.Get("X-KDex-Function-Keep-Schema-Conflict") == "true")
